"""
无人机地面站系统 - Leaflet地图版本
使用开源地图瓦片，在 Streamlit Cloud 上正常显示
"""
...
def create_map():
    ...
    if st.session_state.point_A["set"]:
        ...
        center = [32.2332, 118.7492]
    
    # 创建地图（使用 OpenStreetMap 瓦片，稳定可用）
    m = folium.Map(
        location=center,
    ...
    if st.session_state.point_A["set"]:
        ...
        folium.Marker(
            ...
            <div style="min-width: 150px;">
                ...
                <b>经度:</b> {st.session_state.point_A['lng']:.6f}</p>
            </div>
            """,
        ...
        folium.map.Marker(
            [lat, lng],
            icon=folium.DivIcon(
                html='<div style="font-size: 14pt; color: green; font-weight: bold; text-shadow: 1px 1px white;">A</div>'
            )
        ).add_to(m)
    ...
    if st.session_state.point_B["set"]:
        ...
        folium.Marker(
            ...
            <div style="min-width: 150px;">
                ...
                <b>经度:</b> {st.session_state.point_B['lng']:.6f}</p>
            </div>
            """,
        ...
        folium.map.Marker(
            [lat, lng],
            icon=folium.DivIcon(
                html='<div style="font-size: 14pt; color: red; font-weight: bold; text-shadow: 1px 1px white;">B</div>'
            )
        ).add_to(m)
    ...
    if st.session_state.point_A["set"] and st.session_state.point_B["set"]:
        ...
        folium.PolyLine(
            ...
            weight=5,
            opacity=0.9,
            popup=f"✈️ 规划航线 | 高度: {st.session_state.flight_height}m",
    ...
    # ========== 图例 ==========
    legend_html = '''
    <div style="position: fixed; bottom: 20px; right: 20px; z-index: 1000; background-color: rgba(0,0,0,0.8); padding: 10px 12px; border-radius: 8px; color: white; font-size: 12px; min-width: 130px;">
...
st.sidebar.markdown("- 📏 **左上角**: 测量距离")
st.sidebar.markdown("- 🗺️ **右下角**: 切换图层")
...
if page == "🗺️ 航线规划":
    ...
    # 地图显示
    st.markdown("### 🗺️ 交互式地图")
    st.markdown("💡 **操作提示**：鼠标拖拽平移 | 滚轮缩放 | 右上角全屏 | 左上角测量距离")
...
st.markdown("---")
st.markdown("© 2024 无人机地面站系统 | 底图: OpenStreetMap | 支持 WGS-84/GCJ-02 坐标转换 | 实时心跳监测")
