...
def create_map():
    ...
    if st.session_state.point_A["set"] and st.session_state.point_B["set"]:
        ...
        folium.map.Marker(
            [mid_lat, mid_lng],
            icon=folium.DivIcon(html=f'<div style="background:rgba(0,0,0,0.7);padding:2px 8px;border-radius:20px;color:yellow;">{dist:.0f}m</div>')
        ).add_to(m)
    ...
    # 图例
    legend = '''
    <div style="position:fixed;bottom:20px;right:20px;background:rgba(0,0,0,0.7);padding:8px 12px;border-radius:8px;color:white;font-size:12px;">
...
if page == "航线规划":
    ...
    st.markdown("💡 鼠标滚轮缩放 | 拖拽平移 | 右上角全屏 | 右下角切换图层")
...
st.markdown("---")
st.markdown("© 无人机地面站系统 | 高德卫星图 | WGS-84/GCJ-02 坐标转换 | 实时心跳监测")
